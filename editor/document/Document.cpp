...
Document::InsertParagraphImpl(const std::string &text, std::optional<size_t> position)
{
    if (!position.has_value())
    {
        position = m_documentItems.size();
    }
    m_history.AddAndExecuteCommand(std::make_unique<InsertParagraphCommand>
            (m_documentItems, text, position));
...
void Document::ReplaceText(const std::string &newText, std::optional<size_t> position)
{
    m_history.AddAndExecuteCommand(std::make_unique<ReplaceTextCommand>
            (m_documentItems, newText, position));
}

void Document::ResizeImage(int width, int height, std::optional<size_t> position) {
    m_history.AddAndExecuteCommand(std::make_unique<ResizeImageCommand>
            (m_documentItems, width, height, position));
}

std::shared_ptr<IImage>
Document::InsertImage(const std::string &path, int width, int height, std::optional<size_t> position) {
    return InsertImageImpl(path, width, height, position);
}

std::shared_ptr<IImage>
Document::InsertImageImpl(const std::string &path, int width, int height, std::optional<size_t> position)
{
    m_history.AddAndExecuteCommand(std::make_unique<InsertImageCommand>
            (m_documentItems,path, width, height, position));
...
{
    m_history.AddAndExecuteCommand(std::make_unique<DeleteItemCommand>
            (m_documentItems, index));
}
...
{
    m_history.AddAndExecuteCommand(std::make_unique<SetTitleCommand>
            (m_title, title));
}
